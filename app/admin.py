...
class AlbumModelAdmin(admin.ModelAdmin):
	...
	list_display        = ('title', 'thumb', 'created')
	list_filter         = ('created',)
		...
		if form.is_valid():
			...
			if form.cleaned_data['zip'] != None:
				...
				for filename in sorted(zip.namelist()):
					...
					filepath = '{0}/albums/{1}'.format(image_gallery.settings.MEDIA_ROOT, filename)
					with Image.open(filename) as i:
						img.width, img.height = i.size

					img.thumb.save('thum-{0}'.format(filename), contentfile)
					img.save()
				zip.close()
...
class AlbumImageModelAdmin(admin.ModelAdmin):
	list_display = ('alt', 'image', 'created', 'album')
